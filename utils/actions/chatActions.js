...
export const sendTextMessage = async (
  ...
  const otherUsers = chatUsers.filter((uid) => {
    return uid !== senderData.userId;
  });
...
const sendMessage = async (
  ...
  const app = getFirebaseApp(); // def was needed
  const dbRef = ref(getDatabase(app));
...
export const addUsersToChat = async (
  ...
  const infoMessageText = `${userLoggedInData.firstName} ${userLoggedInData.lastName} added ${userAddedName} ${moreUsersMessage}to the chat`;

  await sendInfoMessage(chatData.key, userLoggedInData.userId, infoMessageText);
};
